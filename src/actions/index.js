...
export const searchByName = name => dispatch => {
  ...
  utilRequest.searchByName(name).then(result => dispatch({
...
export const searchByActor = actor => dispatch => {
  ...
  utilRequest.searchByActor(actor).then(result => dispatch({
...
export const movieAddNew = body => dispatch => {
  ...
  utilRequest.addNewRequest(body).then(result => dispatch({
    ...
    payload: result.data
  })).catch(error => dispatch({type: actionTypes.MOVIE_ADD_NEW_ERROR, payload: error}))
}

export const movieDelete = id => dispatch => {
  dispatch({type: actionTypes.MOVIE_DELETE_PENDING, id});
  utilRequest.movieDeleteRequest(id).then(() => dispatch({
